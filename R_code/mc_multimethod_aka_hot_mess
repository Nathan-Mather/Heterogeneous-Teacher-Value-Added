...
# Set the method for calculation (and specify covariates?)
method <- "wls" # "wls" or "semip" or "qtle"
points <- seq(from=-4, to=4, by=.025) # Evaluation points for semiparametric
ptle <- seq(.01,.99,by=.02) # Percentiles at which to evaluate quantile
...
# Does this need an updated cond function? (Maybe generalize so we don't have to define three times?)
...
out <- foreach(i = 1:nsims, .combine = 'comb', .multicombine = TRUE) %dopar% { # Change %dopar% to %do% if you don't want to run it parallel
  ...
  # Estimate using method of choice
  if(method=="wls"){
  
    # Estimate the weighted VA using WLS
    ww_tab1 <- ww_va(in_data = r_dt, in_weights = opt_weight_type)
    
  } else if(method=="semip"){
    
    # Estimate Value added at each point
    ww_temp <- semip_va(in_data = r_dt, points = points)
    
    # Make a variable with the welfare weight at each point
    if(opt_weight_type == "linear"){
      
      # make weights based on true ex ante ability rather than test one 
      weights_true <- linear_weight_fun(alpha = lin_alpha, in_test_1 = points)
      
    } else if(opt_weight_type == "rawlsian"){
      
      # make weights based on true ex ante ability rather than test one 
      weights_true <- rawlsian_weight_fun(pctile, points)
      
    } else if(opt_weight_type == "equal"){
      
      # make weights based on true ex ante ability rather than test one 
      weights_true <- equal_weight_fun(points)
      
    } else if(opt_weight_type == "v"){
      
      # make weights based on true ex ante ability rather than test one 
      weights_true <- v_weight_fun(v_alpha, points)
      
    } else if(opt_weight_type == "mr"){
      
      # make weights based on true ex ante ability rather than test one 
      weights_true <- mr_weight_fun(mrpctile, mrdist, points)
      
    }
    
    
    # Calculate welfare for each teacher (should this take standardized or nonstandardized VA(x)?)
    # This should be a weird matrix multiplication txpoints X pointsx1
    ww_tab1 <-  ww_temp[ ]* t(weights_true)
    
    
  } else if(method=="qtle"){

    # Estimate Value added at each point
    ww_temp <- semip_va_function()
    
    # Convert into Standard deviations(?)
    ...
    # Calculate welfare for each teacher  (should this take standardized or nonstandardized VA(x)?)
    ww_tab1 <- ww_temp[ , true_ww := sum(teacher_impact * weights_true), "teacher_id" ]
