...
\section{Introduction}
    ...
    % THIS ALLL NEEDS WORK. IT DOESNT REALLY FIT ANYMORE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{The Social Planner Problem}
    ...
    % need to summarize what happens in the appendix and fix up the end bit 
...
\]
 ...
 Measuring heterogeneous impacts along key dimensions can lower the bias. Of course, considering heterogeneity is a bit of a balancing act. Measuring individual impact would be perfect but empirically impossible, but by choosing dimensions that are both policy relevant and ethically relevant (i.e. they vary by both test scores and welfare weights), we may be able to lower the bias significantly. With this in mind, suppose we condition our estimates and welfare weights on some variable $X_i$ (with state space X), for example prior year test scores. Now the bias is equal to:
    ...
    % Note there is something a bit wrong with the integrating over X and it might be worth switching that back to a sum. I thought the integral was more general and matched the rest of what is going on. But I think ther may need to be some additional weight to make it work out? Or is that in the DX? Not enough sleep, need a second opinion.  
    \begin{thm}
    \label{cond_exp_1}
        \begin{align*}
...
%%%%%%%%%%%%%%%%%%% STOPPED THE REDRAFT HERE %%%%%%%%%%%%%%%%%%%%%
    ...
    % I think the rest of this section can all be cut! I am pretty sure it is just flat out wrong due to me being confused about how the X_i vector gets used in practice. Commenting out untill I talk to mike and am more confident. 
    %-------------------------------------------------
