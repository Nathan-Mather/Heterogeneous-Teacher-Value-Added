...
\begin{itemize}
    ...
    \begin{itemize}
        \item {\color{gray}{\citep{ehrenberg1995teachers, dee2005teacher, lockwood2009, Delgado2020, macartney2021quantitative}}}
    \end{itemize}
\end{itemize}
...
Many modelling decisions must be made, including:
    ...
    \begin{enumerate}
        ...
        \item<2-> Student fixed effects or rely on observables?
            \begin{itemize}
                \item<2-> If use student fixed effects in ``level on lags” model in short panel, issue of potential inconsistency % Arellano-Bond correction possible here
            \end{itemize}
...
\begin{frame}{Other Key Issues}

\only<1>{

    The following other key issues were highlighted by \cite{Koedel2015}.
}

\only<2>{
    How biased are VAM models?

    \begin{itemize}
        \item \cite{kane2013have} find that experimental and VAM estimates were close 
        \item \cite{chetty2014measuring1} show bias is not large using several methods
    \end{itemize}
    
    On the other hand, \cite{rothstein2010teacher} finds that future teachers predict current learning. However, this has been raised into question by \cite{goldhaber2015assessing, koedel2011does, guarino2015can}.
}

\only<3>{
    Covariates outside of lagged test scores?
    
    \begin{itemize}
        \item In short, lagged test score is by far the most important
        \item Some studies do suggest that VAM estimates are more accurate with lagged scores from multiple subjects and/or years
    \end{itemize}
}
...
\begin{frame}{Summary of Simulation Results}

    The main takeaways:
    
    \only<1>{
        \begin{itemize}
            \item Our methods consistently recover heterogeneity and ``Welfare Added" and are robust to a wide range of potential issues
            \item Methods require larger samples 30-50+ students per teacher
        \end{itemize}
    }
    
    \only<2>{
        \begin{itemize}
            \item If vertical value added dominates, the ranking of teachers will not differ greatly across our three estimators, but the standard estimates will be more precise
            \item As we increase the relative importance of horizontal value added to vertical value added, the standard value added estimates will incorrectly rank more teachers, whereas our other estimates correctly identify the best teachers for our given policy environment
        \end{itemize}
    }
...
\begin{frame}{Variation in Overall Rankings of Teachers}
    
    \centering
    \includegraphics<1>[width=.8\textwidth]{slides/Figures/ELA_Bin_Overall.png}
    \includegraphics<2>[width=.8\textwidth]{slides/Figures/Math_Bin_Overall.png}
...
\only<1>{
    \centering
    \includegraphics[width=.8\textwidth]{slides/Figures/Example_1220.png}
}

\only<2>{
    \centering
    \includegraphics[width=.8\textwidth]{slides/Figures/Example_1222.png}
}
...
\begin{frame}{Across Teacher Variation in Value Added}

\only<1>{
    \centering
    \includegraphics[width=.8\textwidth]{slides/Figures/Va_vs_Va.png}
}

\only<2>{
    \centering
    \includegraphics[width=.8\textwidth]{slides/Figures/Bin_vs_Bin.png}
}
...
\only<1>{
    ...
    \centering
    \resizebox{.8\textwidth}{!}{\input{slides/Tables/highgrad}}
}

\only<2>{
    Bachelor's Degree 6 years after High School
    \centering
    \resizebox{.8\textwidth}{!}{\input{slides/Tables/bachelor}}
}
